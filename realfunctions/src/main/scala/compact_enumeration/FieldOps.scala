...
object FieldOps {
  ...
  implicit def intervalFieldOps[F: Field: Order]
    : compact_enumeration.FieldOps[spire.math.Interval[F]] =
  ...
  /**
    ...
    */
  implicit def OptFieldStruct[A, F: FieldOps]
    : compact_enumeration.FieldOps[A => Option[F]] {
      val fl: compact_enumeration.FieldOps[F]
    } = new FieldOps[A => Option[F]] {
    val fl = implicitly[FieldOps[F]]
    import FieldOpsSyms._
